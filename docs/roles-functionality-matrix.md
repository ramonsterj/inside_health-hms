...
| Progress notes | Full | None | Work | None | Work | Work | None | Append-only for non-admins. Doctor / nurse / chief nurse can `create` and `read`; only ADMIN holds `progress-note:update`. CHIEF_NURSE gains `progress-note:create` in V096; the legacy `progress-note:update` grant for CHIEF_NURSE in the seed file is revoked in V096. Discharge protection blocks writes for everyone, including ADMIN. |
...
| Nursing notes | Full | None | Work | None | Work | Work | None | Append-only for non-admins. Doctor / nurse / chief nurse can `create` and `read`; only ADMIN holds `nursing-note:update` after V096. Discharge protection blocks writes for everyone, including ADMIN. |
| Vital signs | Full | None | Work | None | Work | Work | None | Append-only for non-admins. Doctor / nurse / chief nurse can `create` and `read`; only ADMIN holds `vital-sign:update` after V097. Discharge protection blocks writes for everyone, including ADMIN. |
