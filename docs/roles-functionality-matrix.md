...
- Nursing notes and progress notes are admin-only update at both the role-permission level (only ADMIN holds `nursing-note:update` / `progress-note:update`) and the service layer (`*Service.updateXxx()` independently asserts ADMIN). Discharge protection blocks updates for everyone, including ADMIN. The `canEdit` field on `NursingNoteResponse` and `ProgressNoteResponse` evaluates to `isAdmin && admission.isActive`. Doctors, nurses, and chief nurses must request edits from an administrator.
- Vital-sign updates follow a different pattern: non-admin users can edit their own records within 24 hours of creation, while the admission is active. ADMIN can edit any record any time on an active admission. This divergence is intentional — see `docs/features/nursing-module.md` revision 1.3 for the rationale.
...
- Chief nurse is not just a label. In the shipped roles it extends nurse access with `patient:update` and `admission:update`. The legacy seed-file grant of `progress-note:update` to CHIEF_NURSE is revoked in V096; CHIEF_NURSE now mirrors the per-record edit policy (admin-only update for both notes types, plus `vital-sign:update` for the vital-sign creator window).
