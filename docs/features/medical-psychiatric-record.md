...
| 1.6 | 2026-05-28 | @author | **Psychology execution scope.** Tightens psychology's relationship with medical orders. PSYCHOLOGIST still cannot create or freely execute orders, but gains the ability to **execute (`mark-in-progress`) and upload result documents for `PRUEBAS_PSICOMETRICAS` orders only** — because the psychologist is the person who administers psychometric tests and produces the report. Grants `medical-order:mark-in-progress` and `medical-order:upload-document` to PSYCHOLOGIST, with a service-layer category guard that rejects either action with `error.medical-order.psychologist.category.scope` when the order's category is anything other than `PRUEBAS_PSICOMETRICAS`. All other categories (medications, labs, referrals, directive) remain off-limits for PSYCHOLOGIST. **Also grants `medical-order:read`** to PSYCHOLOGIST so the cross-admission dashboard endpoint (`GET /api/v1/medical-orders`) and the `/medical-orders` route — both gated by `medical-order:read` — are reachable. The dashboard listing is service-layer scoped to `category = PRUEBAS_PSICOMETRICAS` rows for PSYCHOLOGIST, so the role only sees the orders it can act on; the same scope applies to single-order reads when the caller is PSYCHOLOGIST (returns 404 if the order is in any other category). The frontend dashboard must hide the mark-in-progress and upload-document buttons for PSYCHOLOGIST when the row's category is not `PRUEBAS_PSICOMETRICAS` (defensive only — the scoped read already filters them out). Customer feedback 2026-05-26. |
| 1.7 | 2026-05-28 | @author | **Inventory picker fix on `MedicalOrderFormDialog`.** The picker for "Insumo/servicio a facturar" (`inventoryItemId`) previously called the admin inventory endpoint (`GET /api/v1/admin/inventory-items`), which is gated by `inventory-item:read` — a permission only ADMIN holds. As a result, DOCTOR / NURSE / CHIEF_NURSE / RESIDENT_DOCTOR users saw an empty dropdown when creating a medication order. The form now switches catalogs by selected category: for `MEDICAMENTOS` it loads from the pharmacy endpoint `GET /api/v1/medications` (gated by `medication:read`, which all clinical roles already hold) and shows medications as `"<generic> <strength> (<commercial>)"`; for other billable categories it continues to use the admin inventory endpoint and filters out `kind=DRUG` rows. The catalog is re-fetched whenever the user changes category. No backend, schema, or permission changes — pure frontend fix. Customer feedback 2026-05-26. |
| 1.8 | 2026-06-12 | @author | **Edit guards: immutable category + no edit on terminal orders.** A medical order's `category` is now **immutable on edit** — `updateMedicalOrder` returns `400` if `request.category` differs from the stored category, and `MedicalOrderFormDialog` disables the category select in edit mode (with a "La categoría no se puede cambiar al editar." hint). The medical-record order list now hides the "Editar" affordance for **terminal** orders (`NO_AUTORIZADO` / `RESULTADOS_RECIBIDOS` / `DESCONTINUADO`) — the backend already 400s those edits, and offering Edit exposed a misleading lab "ya fue autorizada y facturada" notice on a merely-rejected order. For lab orders, the non-`SOLICITADO` lab guard now rejects only an actual **provider/test change**; scalar/observation edits on an `AUTORIZADO`/`EN_PROCESO` lab order succeed and leave the snapshotted lines intact. Since category is immutable, the lab→non-lab "clears provider/lines" update branch is removed. Customer feedback. |
...
- **Medical Orders** — append-only for non-admins; only ADMIN can free-form-edit. The order's **category is immutable on edit** (changing it returns `400`; the UI disables the category select in edit mode), and **terminal** orders (`NO_AUTORIZADO` / `RESULTADOS_RECIBIDOS` / `DESCONTINUADO`) reject any update — the UI hides "Editar" for them. State transitions (authorize, mark-in-progress, discontinue, etc.) are exposed as their own permissioned endpoints, not as `update`. **All of these — create, update, and every state transition, plus result-document upload — are blocked once the admission is discharged** (Discharge Protection, 2026-06-01).
...
| Action | Required Role(s) | Permission | Notes |
|--------|------------------|------------|-------|
| View | DOCTOR, NURSE, CHIEF_NURSE, ADMIN, **PSYCHOLOGIST (psychometric only)** | `medical-order:read` | All medical staff can view. PSYCHOLOGIST holds the permission (planned, v1.6) but reads are service-layer scoped to `category = PRUEBAS_PSICOMETRICAS` — both the cross-admission dashboard listing and single-order fetches filter accordingly, so PSYCHOLOGIST never sees medication / lab / referral / directive orders. |
| Create | DOCTOR, ADMIN | `medical-order:create` | Initial state depends on category (see below) |
| Update | ADMIN | `medical-order:update` | Append-only for doctors. Category is immutable (`400` on change); terminal orders reject any update; non-`SOLICITADO` lab orders reject provider/test changes but allow scalar/observation edits. |
...
| Mark in progress | NURSE, DOCTOR, CHIEF_NURSE, ADMIN, **PSYCHOLOGIST (psychometric only)** | `medical-order:mark-in-progress` | Transitions `AUTORIZADO → EN_PROCESO`. Only valid for results-bearing categories (labs / referencias / pruebas psicométricas). Means "sample taken / patient referred / test administered" — after this, discontinue is no longer allowed. **AUXILIARY_NURSE is excluded** (V117 grants no `medical-order:mark-in-progress`); `MedicalOrderService.markInProgress` also guards with 403 `error.nursing.auxiliary.denied` for auxiliary-only callers. PSYCHOLOGIST holds the permission but the service rejects the call when `category != PRUEBAS_PSICOMETRICAS` (see v1.6). |
| Upload result document | DOCTOR, NURSE, ADMIN, **PSYCHOLOGIST (psychometric only)** | `medical-order:upload-document` | Allowed only on results-bearing categories and only while the order is in `AUTORIZADO`, `EN_PROCESO`, or `RESULTADOS_RECIBIDOS`. The first upload auto-transitions the order to `RESULTADOS_RECIBIDOS` (skipping `EN_PROCESO` if the order was still in `AUTORIZADO`). **AUXILIARY_NURSE is excluded** (V117 grants no `medical-order:upload-document`); `MedicalOrderDocumentService.uploadDocument` also guards with 403 for auxiliary-only callers. PSYCHOLOGIST holds the permission but the service rejects the call when `category != PRUEBAS_PSICOMETRICAS` (see v1.6). |
...
- **The order card communicates authorization status from `status`, primarily and unambiguously.** For auth-required categories the card renders an explicit pending-authorization indicator ("Pendiente de autorización") while `SOLICITADO`, so a not-yet-authorized order is never read as approved. The card's left-border accent is **status-driven** — it reuses the same `SEVERITY_BY_STATUS` map as `MedicalOrderStateBadge` (`SOLICITADO` → neutral, `NO_AUTORIZADO` → danger/red, `AUTORIZADO`/`ACTIVA` → green) — instead of a static green. The "Autorizado por …" audit line remains visible through normal post-authorization states such as `EN_PROCESO` and `RESULTADOS_RECIBIDOS`, but is suppressed when an order authorized and later discontinued would otherwise still present as currently authorized.
