...
| 1.8 | 2026-05-29 | @author | FEFO is now **warehouse-scoped** and per-lot on-hand stock lives in `inventory_warehouse_stock` (the legacy `inventory_lots.quantity_on_hand` column was dropped by V121). Dispensing selects the soonest-to-expire non-empty lot **within the caller's warehouse**, and the expiry dashboard gains a warehouse filter. See [`warehouse-inventory-management.md`](warehouse-inventory-management.md). |
| 1.9 | 2026-05-29 | @author | **Catalog `quantity` is the system-wide total — now displayed as such.** `MedicationResponse.quantity` sums on-hand across every bodega; since dispensing is warehouse-scoped, the pharmacy detail view now also returns a per-warehouse `warehouseStock` breakdown (FR-11 in [`warehouse-inventory-management.md`](warehouse-inventory-management.md)) and the list/detail label the total as "all bodegas" so clinical viewers are not misled. Read-model + labeling only. |
