...
> **Nota de mantenimiento (2026-05-28):** la matriz en espanol esta pendiente de incorporar el rol `RESIDENT_DOCTOR` (V114/V115, ya en produccion) y los dos roles planificados `AUXILIARY_NURSE` y `MAINTENANCE`. Ver la version en ingles ([roles-functionality-matrix.md](roles-functionality-matrix.md)) para la matriz completa y las especificaciones [`nursing-roles-split.md`](features/nursing-roles-split.md) y [`warehouse-inventory-management.md`](features/warehouse-inventory-management.md). Esta seccion se actualizara cuando los roles planificados se entreguen.
...
- **`AUXILIARY_NURSE` (planificado):** rol restringido. Puede crear y leer notas de enfermeria y signos vitales, y leer expediente medico/ordenes/medicamentos. **No** puede administrar medicamentos, marcar ordenes `EN_PROCESO`, subir documentos de resultado, ni redactar evoluciones. El `NURSE` existente se interpreta como "enfermero graduado".
