...
- **`RESIDENT_DOCTOR` (ya entregado, V114/V115):** clona todos los permisos de `DOCTOR` y ademas tiene `admission:create`. A diferencia de `DOCTOR`, ve todos los pacientes y admisiones del hospital (no esta limitado a los suyos). El servicio de admision asigna automaticamente el `resident_id` al usuario autenticado cuando el usuario es residente. `ADMIN` es la unica excepcion: no necesita portar `RESIDENT_DOCTOR` y debe seleccionar explicitamente el residente (`residentId`) al registrar una admision. V119 revoca `RESIDENT_DOCTOR` del `admin` semilla; cualquier otro rol con `admission:create` pero sin `RESIDENT_DOCTOR` queda bloqueado con `error.admission.resident.role.required`.
