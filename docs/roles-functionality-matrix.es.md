...
- Las notas de enfermeria y las notas de evolucion son admin-only para `update`, tanto a nivel de role-permission (solo ADMIN tiene `nursing-note:update` / `progress-note:update`) como a nivel de servicio (`*Service.updateXxx()` valida ADMIN de forma independiente). La proteccion al alta bloquea las actualizaciones para todos, incluido ADMIN. El campo `canEdit` en `NursingNoteResponse` y `ProgressNoteResponse` se evalua como `isAdmin && admission.isActive`. Doctores, nurses y chief nurses deben solicitar la edicion a un administrador.
- Los signos vitales siguen un patron distinto: los no admin pueden editar sus propios registros dentro de 24 horas de creacion, mientras la admision siga activa. ADMIN puede editar cualquier registro en cualquier momento si la admision esta activa. Esta divergencia es intencional — ver `docs/features/nursing-module.md` revision 1.3 para la justificacion.
...
- Chief nurse no es solo una etiqueta. En los roles integrados extiende a nurse con `patient:update` y `admission:update`. El grant legado de `progress-note:update` para CHIEF_NURSE en el seed se revoca en V096; CHIEF_NURSE ahora respeta la politica de edicion por registro (admin-only para ambas notas, mas `vital-sign:update` para la ventana de creador en signos vitales).
