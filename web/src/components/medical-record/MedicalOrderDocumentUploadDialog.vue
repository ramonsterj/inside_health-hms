...
import {
  uploadMedicalOrderDocumentSchema,
  type UploadMedicalOrderDocumentFormData
} from '@/validation/medicalOrderDocument'
...
const { defineField, handleSubmit, errors, resetForm, setErrors, setFieldValue } =
  useForm<UploadMedicalOrderDocumentFormData>({
    validationSchema: toTypedSchema(uploadMedicalOrderDocumentSchema),
    initialValues: {
...
const onSubmit = handleSubmit(async values => {
  try {
    ...
    showSuccess('medicalRecord.medicalOrder.documents.uploaded')
    closeDialog()
    emit('uploaded')
  } catch (error) {
...
<template>
  <Dialog
    ...
    :header="t('medicalRecord.medicalOrder.documents.uploadTitle')"
    modal
    :style="{ width: '500px' }"
    @hide="closeDialog"
  >
