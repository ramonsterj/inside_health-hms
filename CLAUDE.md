...
- ✅ Nursing Module (nursing notes — admin-only update with discharge protection; vital signs — BP, HR, RR, temperature, oxygen saturation, glucometría — admin-only update with discharge protection, append-only for non-admins per V097). Two operational nursing roles: `NURSE` (graduate nurse — full clinical scope) and `AUXILIARY_NURSE` (V117 — vital signs + nursing notes only; cannot administer meds, mark orders in progress, or upload result documents). `CHIEF_NURSE` is the supervisory superset of NURSE. See docs/features/nursing-roles-split.md.
