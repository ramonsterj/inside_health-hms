...
- ✅ Bed Occupancy View (aggregated `GET /api/v1/rooms/occupancy` endpoint, gated by `room:occupancy-view` permission granted to ADMIN/NURSE/CHIEF_NURSE/AUXILIARY_NURSE/ADMINISTRATIVE_STAFF and RESIDENT_DOCTOR (V118) — explicitly NOT to plain DOCTOR; counts only ACTIVE non-deleted HOSPITALIZATION admissions)
...
- ✅ Bed Occupancy View UI (`/bed-occupancy`: gender groups rendered as side-by-side columns — women left, men right, collapsing to stacked sections on narrow viewports; per-room cards with one bed slot per capacity, summary header, filters by sex/type/status, search by room number or patient name, 30s auto-refresh paused on hidden tab, "Admit here" deep-link that pre-selects the room in the admission wizard via `?roomId=`. This is the **default landing page** for nurses (NURSE/CHIEF_NURSE/AUXILIARY_NURSE) and resident doctors — `/dashboard` redirects them here; ADMIN is never auto-redirected)
